...
router.post("/", withAuth, async (req,res) => {
    try {
        const postData = await Post.create({title: req.body.title, body: req.body.body, user_id: req.session.user_id});
        if (!postData) {
            res.status(400)
...
router.put("/:id", withAuth, async (req, res) => {
    try {
        const [affectedPosts] = await Post.update(req.body, {where: { id: req.params.id }});
        if (!affectedPosts || affectedPosts == 0) {
            res.status(400)
            .json({ message: "Failed to edit post!" });
            return;
        }
